...
{
    QString ret = QString("blood:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BOD unit input in lnReqAmount, we generate the URI with BOD as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BOD, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
